...
class LlmPuzzleService {
  ...
  async greetPlayer(playerName?: string): Promise<PuzzleMessage> {
    ...
    if(coin){
      ...
      Будь дружелюбным. Используй crypto мемы и приколы. Отвечай на английском языке.
      ...
      prompt = `Ты устал от вечных наплывов пользователей, и не хочешь их больше видеть, пошути на тему not eligible, dump рынка, что Monad скам, 
        Full Access роль не мультипликатор, и вообще MikeWeb заберет дроп себе. Не извиняйся за свои шутки. Не делай слишком большой ответ. Отвечай на английском языке.
        Верни JSON вида { "text": "...", "tone": "encourage" }`;
    }
  ...
  async moveComment(
    ...
    if (isCorrect) {
      if (coin) {
        ...
        Если ход был решающим, то вместо комментария поздравь игрока, который успешно решил шахматный паззл. Отвечай на английском языке.
      ...
      if(coin){
        ...
        Отвечай на английском языке. Тон — encourage, Верни JSON { "text": "...", "tone": "encourage" }`;
      }
    }
