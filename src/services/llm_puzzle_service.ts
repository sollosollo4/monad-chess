...
class LlmPuzzleService {
  ...
  async moveComment(
    ...
    if (isCorrect) {
      if (coin) {
        // Доброе/дружелюбное
        prompt = `Пиши in English. Скажи короткий комментарий к правильному ходу ${move} на шаге ${step + 1} из ${total}.
      ...
      if(coin){
        prompt = `Пиши in English. Скажи короткий комментарий к ошибочному ходу ${move} на шаге ${step + 1} из ${total}. 
  ...
  async puzzleInstruction(themes: string[]): Promise<PuzzleMessage> {
    ...
    const prompt = `Пиши in English. У тебя есть шахматный паззл с темами: [${joined}]. 
